...
import { useGetAllClassesQuery } from "@/redux/api/classesApi.js"; // 🔥 class list anar jonno
...
const FeesPage = () => {
  ...
  const { data: classData } = useGetAllClassesQuery(); // 🔥 real class list
  const [create] = useCreateFeesMutation();
  const [update] = useUpdateFeesMutation();
  const [remove] = useDeleteFeesMutation();

  // 🔥 Class dropdown options — real classId + name
  const classOptions = useMemo(() => {
    return (classData?.data || []).map((c: any) => ({
      label: c.name,
      value: c._id,
    }));
  }, [classData]);

  // 🔥 ফিল্টারিং লজিক — fixed: fee.studentId already populated object,
  // tai studentData te ager moto find kora lagena
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-orange-50/30 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <InvoiceModal
          ...
          fee={selectedFee}
          student={resolveStudent(selectedFee)}
        />
      </div>
