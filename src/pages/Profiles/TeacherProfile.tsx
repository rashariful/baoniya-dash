...
import { useMyInfoQuery } from "../../redux/api/userApi"; // path adjust korben
...
const colorCycle = ["blue", "indigo", "purple", "pink", "green"];
...
const TeacherProfile = () => {
  const { data, isLoading, isError } = useMyInfoQuery();
  ...
  if (isError || !data?.data?.profile) {
    return <div className="text-center py-20 text-red-500">Profile load korte problem hocche.</div>;
  }

  const user = data.data;
  const t = data.data.profile;

  const teacher = {
    picture: t.thumbnail || "https://via.placeholder.com/150",
    name: t.name,
    designation: t.designation,
    qualification: t.qualification || "N/A",
    joinDate: formatDate(t.joinDate),
    schoolJoinDate: formatDate(t.schoolJoinDate),
    ...
    contact: {
      ...
      bloodGroup: t.bloodGroup || "N/A",
      emergencyContact: t.emergencyContact || { name: "N/A", phone: "N/A" },
      social: t.social || {},
    },
    education: (t.education || []).map((edu, idx) => ({
      id: idx.toString(),
      label: edu.label,
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden my-8">
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-amber-50 rounded-xl p-5 border border-amber-100">
              ...
              <p className="text-base font-medium text-gray-800">{teacher.contact.emergencyContact.name || "N/A"}</p>
              <p className="text-sm text-gray-600">{teacher.contact.emergencyContact.phone || "N/A"}</p>
            </div>
          </div>
        </div>
        ...
        {/* Education - empty hole message dekhabe */}
        <div>
          <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2">
            <span className="text-indigo-600">🎓</span> Education
          </h2>
          {teacher.education.length === 0 ? (
            <p className="text-gray-500 text-sm">Education info added hoyni ekhono.</p>
          ) : (
