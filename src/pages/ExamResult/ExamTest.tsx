...
//   const [searchTerm, setSearchTerm] = useState('');
//   const [selectedClassId, setSelectedClassId] = useState('');
//   const [statusFilter, setStatusFilter] = useState('All'); // All | Pass | Fail | Absent
//   const resultRef = useRef(null);
...
//     });
//   }, [examResults, searchTerm, selectedClassId, statusFilter]);
...
//     return (
//       <div ref={resultRef} className="bg-white p-8 max-w-4xl mx-auto shadow-lg rounded-xl">
//         {/* School Header */}
...
//               {subject.subjectId?.name || 'N/A'}
//             </td>
//             <td className="px-4 py-2 text-sm text-center">{subject.written ?? 0}</td>
//             <td className="px-4 py-2 text-sm text-center">{subject.mcq ?? 0}</td>
//             <td className="px-4 py-2 text-sm text-center">{subject.ca ?? 0}</td>
//             <td className="px-4 py-2 text-sm text-center">{subject.practical ?? 0}</td>
...
//     <p className="text-gray-500 text-lg">কোনো বিষয়ের নম্বর এখনো এন্ট্রি করা হয়নি</p>
//     <p className="text-sm text-gray-400 mt-1">Subjects array is empty → GPA = 0</p>
//   </div>
// )}
...
//             className="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[160px]"
//           >
//             <option value="">All Classes</option>
//             {(classData?.data || []).map((cls) => (
//               <option key={cls._id} value={cls._id}>
...
//           Showing <span className="font-semibold">{filteredResults.length}</span> of {examResults.length} students
//         </div>
//       </div>
...
//         {filteredResults.map((result) => {
//           const className = classNameMap[result.studentId?.classId] || 'N/A';
//           return (
//             <div
//               key={result._id}
